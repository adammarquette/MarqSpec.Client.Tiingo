...
Find defects **before they reach `develop`**, in a public data-only client whose token and error semantics
the consumer's news adapter sits on. You **report**; you do not fix — reviewing and repairing in one pass
loses the independence that makes review worth running, and an author who never sees the finding never
learns the pattern.
...
[QA contract](../../MarqSpec.Client.Tiingo.IntegrationTests/AGENTS.md).
...
> **Can this change put the API token in a URL, a log line, or an exception message — or swallow a
...
regardless of how clean the rest reads. Tiingo authenticates with `Authorization: Token` (R-5); a token in
the URL is always a finding here.
...
- **Stale documentation is a finding** — an XML doc advertising an obsolete contract, a README documenting a
  method that no longer exists. This repo has shipped both (a `Models/` folder and a `.sln` that were never
  here). On transport paths a false claim is worse than no claim.
